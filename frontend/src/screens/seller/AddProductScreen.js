/**
 * Add Product Screen  –  Module 3: Product Management (Seller)
...
export default function AddProductScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await createProduct({ title: title.trim(), description: description.trim(), price: Number(price), category, images });
      Alert.alert('Submitted! ✅', 'Your product is pending admin approval.', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (err) {
      Alert.alert('Error', err.response?.data?.message || 'Failed to add product. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Screen>
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView style={styles.scrollView} keyboardShouldPersistTaps="handled">
          <Text style={[styles.title, { color: isDark ? colors.textOnDark : colors.slate800 }]}>Add Product</Text>
          ...
          <Card>
            ...
            <Input label="Description" placeholder="Describe your product in detail" value={description} onChangeText={(t) => { setDescription(t); setErrors((e) => ({ ...e, description: '' })); }} error={errors.description} editable={!loading} multiline numberOfLines={4} />
            <Input label="Price (₹)" placeholder="0" value={price} onChangeText={(t) => { setPrice(t); setErrors((e) => ({ ...e, price: '' })); }} error={errors.price} editable={!loading} keyboardType="decimal-pad" />
          ...
          <Card>
            ...
            >
              ...
              <Text style={[styles.imagePickerText, { color: isDark ? colors.mutedDark : colors.mutedLight }]}>
                {images.length >= MAX_IMAGES ? 'Maximum 5 images' : pickingImg ? 'Loading...' : 'Tap to add images'}
              </Text>
              <Text style={[styles.imagePickerSubtext, { color: isDark ? colors.mutedDark : colors.mutedLight }]}>JPEG, PNG, WebP · Max 1MB each</Text>
            </TouchableOpacity>
          </Card>
          ...
          <View style={[styles.notice, { backgroundColor: isDark ? 'rgba(120,53,15,0.3)' : colors.accent50, borderColor: isDark ? colors.accent800 : colors.accent200 }]}>
            ...
            <Text style={[styles.noticeText, { color: isDark ? colors.accent300 : colors.accent700 }]}>
              Your product will be reviewed by an admin before it goes live on the marketplace.
            </Text>
          </View>

          <Button title="Submit for Approval" onPress={handleSubmit} loading={loading} />
...
const styles = StyleSheet.create({
  ...
  imagePicker: { borderWidth: 2, borderStyle: 'dashed', borderRadius: 12, paddingVertical: 20, alignItems: 'center' },
  imagePickerIcon: { fontSize: 24, marginBottom: 4 },
  imagePickerText: { fontSize: 14 },
  imagePickerSubtext: { fontSize: 11, marginTop: 2 },
